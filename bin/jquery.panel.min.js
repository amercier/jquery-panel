...
if(!data||!data._init){var allOptions=$.extend({_init:true},defaultOptions,data||{},options),outerWidth=$this.outerWidth(),outerHeight=$this.outerHeight(),width=$this.width(),height=$this.height(),position=$this.position(),cssPosition=$this.css("position"),top=$this.css("top")||"auto",bottom=$this.css("bottom")||"auto",left=$this.css("left")||"auto",right=$this.css("right")||"auto",side=allOptions.side;
...
var wrapper=$this.wrap("<div></div>").parent().addClass("panel");wrapper.css({top:top,bottom:bottom,left:left,right:right,height:top!=="auto"&&bottom!=="auto"?"auto":(/%$/.test(height)?height:outerHeight),width:left!=="auto"&&right!=="auto"?"auto":(/%$/.test(height)?height:outerWidth),margin:[$this.css("margin-top")||0,$this.css("margin-right")||0,$this.css("margin-bottom")||0,$this.css("margin-left")||0].join(" "),position:cssPosition});
